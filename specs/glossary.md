...
- **Red-green evidence**: One exact `Verify:` command recorded failing for the absent unit outcome at a clean pre commit and passing at a later clean implementation commit. Review reproduces both runs and checks `HEAD`; the CLI validates only receipt structure.
