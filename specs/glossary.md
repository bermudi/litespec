...
- **Evidence receipt**: Verbatim record required before ticking a unit: the exact `Verify:` command, labeled `sha:` (full 40- or 64-char hex), labeled `exit status:`, nonempty fenced output, and `Evidence scope: this command exited <status> at <sha>; nothing else is inferred.` A nonempty `Evidence:` label is not a receipt. Validate checks structure; review re-runs.
