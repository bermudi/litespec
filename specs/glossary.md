...
- **Unit**: One demo-able outcome per GH issue `##` with `Done means:` and `Verify:` that must fail without the outcome. Its rebuild-routing identity is its exact heading plus its positive 1-based occurrence among units with that heading. Built one at a time; a checked GitHub unit remains selectable while that identity has an unresolved rebuild request. No `tasks.md`.
