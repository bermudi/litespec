...
The review skill description in `internal/paths.go` MUST reflect the context-aware behavior. The description SHALL mention artifact review (pre-implementation), implementation review (during implementation), and pre-archive review (post-implementation). The skill ID MUST be `review` and the skill name MUST be `litespec-review`.
