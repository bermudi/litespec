...
Identity fields carry the post-amendment identity because heading is itself a contract field an amendment may rename; `Old digest:` is the only link to the superseded text and what makes a rename auditable rather than paradoxical. An amendment is an unresolved rebuild request resolved only by a later complete identity-bearing receipt whose `unit digest:` equals `New digest:`. Observed receipt digests must form a chain over amendment edges ending at the current contract digest: two amendments between receipts bridge through their shared intermediate digest, while an edit-plus-repost with no bridging amendment is a visible validation error. This extends `0005-review-reopens-units`; where that ruling reopened units without touching the body, this ruling governs who may move the target those bodies record.
