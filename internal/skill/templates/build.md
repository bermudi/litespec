...
8. Post the receipt and tick the box (`- [x]`) only after evidence is posted:
   - GH issue queue: post the receipt as an issue comment. GitHub caps issue comments at 65,536 characters; when the complete receipt exceeds the cap, split it at a field boundary (after a scope line, never inside a fence), end every full comment with the exact line `Receipt continues in next comment (GitHub comment size limit).`, and post the remaining fields as the immediately following comment, chaining as needed — never truncate or summarize fenced output. For an initially unchecked unit, then check its box in the issue body. For a checked rebuild, leave the issue body and prior comments unchanged; the fresh identity-bearing receipt resolves its requests.
