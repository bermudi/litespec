...
- **New feature — plan fuzzy → clear:**
  ...
  ```
  `litespec-plan` starts from a clean tree, creates `litespec/<change-name>`, and records `Base:` + `Branch:` with proposal, design, and units in the labeled GH issue body (or local queue). `litespec-build` implements one unit at a time on that branch. `litespec-review` safely screens tracked and untracked issue-owned paths before reading them. Close only when every unit is checked and review returns `PASS`.
