...
4. **Build one unit** — `litespec-build` implements one `## <outcome>` from the issue. It satisfies `Done means:` and `Verify:`, posts a complete evidence receipt, ticks the checkbox, commits, and stops.
...
| `litespec-build` | One unit at a time. Satisfy `Done means:` and `Verify:`, post a complete evidence receipt, tick the box, commit, stop. |
