...
Review routing is ordered: suggestion; unit violation; in-scope finding outside units; out-of-scope finding. DISPUTED is terminal non-blocking. CRITICAL/WARNING inside issue scope blocks even when no unit covers it. Evidence cross-check (red-green receipt, pre→post→HEAD ancestry, three re-runs) remains part of review. Review first constructs an independent risk inventory, then consults append-only HEAD-keyed coverage records and persists a new advisory record. A unit violation produces a rebuild request only while fewer than two rebuild cycles completed against the current unit digest. The next violation records `Re-plan required:` and plan must amend/reshape that contract before build continues. GitHub metadata lives in comments; local metadata is appended after all units in a clean metadata commit. The issue closes only when every unit is checked, every rebuild request, re-plan marker, and amendment is resolved, review returns `PASS`, and the issue's `Branch:` is merged (decision 0008) — merge first, then close, so a closed issue leaves no work stranded on a branch.
...
Every `Done means:` clause is a bullet carrying a unique bracketed ID. Required `Scenarios:` entries map every clause ID to at least one named test scenario. Filesystem, process, and network units declare `Boundary:` and a `Risk cases:` matrix covering timeout, cleanup, non-ENOENT errors, concurrency, and optional configured dependencies; each risk maps to a scenario ID or gives a concrete N/A reason. Validate checks mapping shape and references, while plan/review judge whether boundaries were omitted and tests are adequate. Optional `Read first:`, `Constraints:`, and `Depends:` retain their current meanings. All contract fields participate in the unit digest. New receipts use the ordered `Protocol: evidence/v2`, `Digest algorithm: unit-contract-sha256-v1`, and bounded `receipt-sha256-v2:` Receipt ID headers — the ID's canonical fields take the excerpt text, byte counts, and full-output SHA-256s instead of the raw outputs, so the same run yields the same ID and appending passing tests no longer changes what the issue must hold; receipts declaring `evidence/v1` keep the v1 header and full-output ID, and complete unversioned receipts remain on the legacy parser and derive an in-memory legacy ID. Superseded receipts are validated against their own declared protocol, Verify command, and digest algorithm, with raw-output chunks repeating the parsed receipt identity; only an explicit amendment chain may connect them to the current contract, while current-digest receipts must match current Verify exactly.

Evidence protocol remains one exact command: build records a meaningful non-zero run on a clean pre commit, creating at most one verifier-only commit when needed, then creates one or more implementation/fix commits. Post is the final clean commit where `Verify:` passes. New receipts declare their parser and digest versions immediately after `Evidence:`; their content-derived IDs exclude wrappers and continuation boundaries — for v2 the full output participates only as byte count and SHA-256 — while unknown or partial metadata never falls back and legacy receipts remain readable. Review replays Verify in a detached temporary worktree at pre, post, and a detached temporary worktree at `HEAD`; all are removed even when Verify fails. Red-green evidence does not prove that Verify targets the right behavior, so scenario mapping and adversarial review remain required. A unit may complete at most two review-requested rebuild cycles against one current contract digest. On the next unit-breaking finding, review records a digest-bound `Re-plan required:` marker instead of another rebuild request. Build refuses that marker. Plan resolves it only through an amendment from the marked digest, narrowing/renaming the unit and appending units as needed. The amendment resets rebuild counting for its new digest and remains unresolved until fresh evidence; that amendment evidence does not consume a review-requested rebuild cycle. Before any unit is filed, plan dry-runs its Verify on the base tree: an honest result is non-zero (outcome or verifier missing) or green only for a test run where every named file actually executed, because test runners silently skip named files that do not match their discovery patterns. Newly assembled receipts declare `Protocol: evidence/v2` and carry bounded excerpts: a deterministic head excerpt, one literal elision marker naming the exact elided byte count, and a tail excerpt with byte-exact, rune-boundary-safe cuts, beside the full output's byte count and SHA-256. One receipt fits one 8192-byte comment (head budget 2048, tail budget 3072 — fixed constants, never configuration), never continues or chunks, and assembly refuses visibly — naming the oversized field — when identity and status fields alone exceed the budget (decision 0012). Validation dispatches on the declared protocol: v2 receipts must be arithmetically consistent and unelided fences must hash to the declared SHA-256, while receipts declaring `evidence/v1` or the legacy shape keep their retained parsers — a receipt that exceeds GitHub's 65,536-character comment limit continues into the immediately following comment: fields split after a scope line; a raw pre/post block that alone exceeds the limit uses repeated identity-bearing `Raw output chunk:` records with consecutive `Chunk: <n>/<total>` fields and closed fenced payloads concatenated without inserted bytes. Each full comment ends with the literal marker `Receipt continues in next comment (GitHub comment size limit).`; fenced output is never truncated. Validate joins only the literal next comment before scanning; a blank/intervening, dangling, duplicated, misordered, or wrong-identity continuation is a visible incomplete-receipt error. Before filing, plan cross-checks each unit outcome against earlier units in the same queue: an outcome an earlier unit delivers or is constrained to preserve is never re-delivered — it becomes a regression-pin unit whose outcome is the named tests, with a Verify that fails while the pin is absent, or it is dropped.
