...
Each queue issue owns a dedicated `litespec/<change-name>` branch. `plan[clear]` requires a clean tree, captures `Base:`, creates the branch, and records `Branch:`. Review refuses a mismatch, enumerates tracked and untracked paths without contents, and screens names and file types before reading. Secret-like paths, symlinks, and non-regular files stop review. Unrelated work uses another branch or worktree.
