...
- **`litespec receipt`** assembles one validator-clean evidence receipt for a resolved queue unit: it resolves the unit by exact heading and positive same-heading occurrence (digest identity semantics), reads raw run outputs from `--pre-out`/`--post-out` files, enforces red-green invariants plus `git merge-base --is-ancestor` ancestry before writing anything, and emits a single bounded `receipt-0001.md` comment file (into `--out <dir>` when given, else the current directory; emitted paths and printed commands are absolute so `--post` and pasted commands work from any directory, with the body-file argument shell-quoted when needed; a failed comment-file write leaves no partial set) that pass the existing evidence grammar by construction; newly assembled receipts declare `Protocol: evidence/v2` and carry bounded excerpts beside each run output's byte count and SHA-256 in a single comment, while the decision-0008 continuation and chunk machinery remains only for retained earlier-protocol receipts (decision 0012). `--issue <N>` also prints the exact `gh issue comment` commands in posting order; `--queue <path>` emits files only. Posting is strictly opt-in: `--post` runs the printed commands itself in posting order, reporting each posted comment, and a failing gh invocation stops the chain naming the failing command, the posted parts, and the unposted files with no retry or reordering — the CLI's first gh write path. It never ticks checkboxes — the build skill's hand-assembly protocol remains valid until a later change mandates the CLI. Optional `--rebuild` and `--recovered-from` assemble rebuild routing identity and recovery provenance into the same canonical Receipt ID
