...
- **GH issues are the change/queue** — GH issue body holds proposal + design + queue (units with identified Done means clauses, Scenarios, Verify, and optional Read first / Constraints / Depends / Boundary + Risk cases) + immutable `Base: <sha>` and `Branch: litespec/<change-name>` ownership lines. `plan[clear]` starts clean and creates the dedicated branch; all work on it belongs to the issue. Offline fallback is `specs/queues/<name>.md`.
- **Units** — one external boundary or one failure policy per `##`, not merely one broad demo. Every bracket-identified `Done means:` clause maps through `Scenarios:` to a named test scenario. Filesystem/process/network units declare `Boundary:` and account in `Risk cases:` for timeout, cleanup, non-ENOENT errors, concurrency, and optional configured dependencies by scenario ID or reasoned N/A. One exact `Verify:` must fail without the outcome, and plan dry-runs it on the base tree before filing (non-zero, or green with every named test file executed); a unit whose outcome an earlier unit already delivers or is constrained to preserve is reshaped as a regression pin or dropped before filing. Checked units carry a complete red-green receipt whose `unit digest:` binds all contract fields; new receipts declare `Protocol: evidence/v2` with `Digest algorithm: unit-contract-sha256-v1` and a bounded content-derived `Receipt ID:` while receipts declaring `evidence/v1` or the legacy unversioned shape remain on their retained parsers; superseded receipts are validated against their own declared protocol, Verify command, and digest algorithm and only an explicit amendment chain may connect them to the current contract, while current-digest receipts must match current Verify exactly. Newly assembled receipts carry bounded excerpts — head, one literal elision marker naming the exact elided byte count, tail — beside the full output's byte count and SHA-256, fitting one 8192-byte comment; they never continue or chunk, and assembly refuses visibly when identity and status fields alone exceed the budget (decision 0012). Retained-protocol receipts that exceed GitHub's comment-size limit continue into the immediately following comment (field-boundary split or explicit identity-bearing raw-output chunks, literal continuation marker, never truncated) and validate joins only the literal next comment. Pre is a clean non-zero missing-outcome run; if needed build creates one verifier-only commit. Build then creates one or more implementation/fix commits; post is the final clean commit where `Verify:` passes. Review replays pre/post/HEAD in a detached temporary worktree at each commit, including a detached temporary worktree at `HEAD`; all are removed even when Verify fails. Red-green discrimination does not prove that Verify targets the right behavior. A rebuild receipt resolves earlier requests for its exact unit identity. After two completed review-requested rebuild cycles against one digest, the next unit-breaking finding records `Re-plan required:` and plan must reshape the contract through an amendment before build continues. Amendments reset the cycle count for their new digest and remain unresolved until fresh evidence; amendment evidence is not a review-requested rebuild cycle. Review also writes append-only HEAD-keyed coverage records; later reviewers use them to expand an independently drafted risk inventory, never as proof. GitHub metadata lives in comments; local metadata is appended after all units in a clean metadata commit. Built one unit at a time. No `tasks.md`
...
- `build` implements one unit per session: from a clean tree it runs the exact `Verify:` before implementation and requires a meaningful non-zero pre run. If the verifier does not exist, it may create at most one verifier-only commit. It selects unchecked units or checked units with unresolved rebuild requests or amendments, but refuses unresolved re-plan markers. It treats scenario/risk mappings as fixed contract fields, creates immutable implementation/fix commits, and records post at the final clean commit where `Verify:` passes. It posts both runs as one bounded evidence/v2 receipt carrying the unit digest from `litespec digest`; local-queue bookkeeping is a separate metadata commit. Stops
- `review` is adversarial — it independently inventories risks, then consults prior append-only coverage records to expand unexercised areas without treating them as proof. It cross-checks every checked unit's receipt and ancestry and replays Verify in detached pre/post/HEAD worktrees. For a unit-breaking finding it requests rebuild only while fewer than two rebuild cycles completed against the current digest; the next finding records a blocking re-plan marker. Review persists a HEAD-keyed coverage record before returning its verdict. Red-green discrimination does not prove that Verify targets the right behavior; review still judges that. (See below.)
- GH issue is the queue: each unit is `## <outcome>` with identified `Done means:` clauses, `Scenarios:`, `Verify:`, and a status checkbox; optional `Read first:` / `Constraints:` / `Depends:`; and `Boundary:` + `Risk cases:` for filesystem/process/network units. Checked units carry complete evidence. GitHub metadata is append-only comments; local metadata is appended after all units
...
2. **CRITICAL or WARNING breaking a unit contract** → blocking rebuild via `build` until two rebuild cycles complete against the current digest; the next finding blocks on `plan` contract reshaping instead.
...
- **Unit = boundary or failure policy + scenarios + failing Verify** — each identified outcome clause maps to a named test scenario. Filesystem/process/network units account for the standard negative-risk matrix. One exact Verify gates build
...
- **Validate structure, not semantics** — the CLI validates structural contracts (syntax, references, mappings, spec format) and successful output says implementation semantics were not verified. Do not encode heuristic semantic checks that compensate for model limitations; fix repeated model gaps in the relevant skill
- **Review routes exhaustively** — suggestion; unit violation; in-scope outside-unit finding; out-of-scope finding. In-scope CRITICAL/WARNING blocks even without a unit; non-trivial work becomes a unit on the existing parent branch. Out-of-scope work routes to a later `plan[clear]` and its own branch. Review coverage is advisory metadata, not a finding tracker
- **Repeated rebuilds force reshaping** — after two completed review-requested rebuilds against one unit digest, the next unit-breaking finding routes to plan and requires an amendment before build. See decision `0007-repeated-rebuilds-force-contract-reshaping`
...
- **`litespec receipt`** assembles one validator-clean evidence receipt for a resolved queue unit: it resolves the unit by exact heading and positive same-heading occurrence (digest identity semantics), reads raw run outputs from `--pre-out`/`--post-out` files, enforces red-green invariants plus `git merge-base --is-ancestor` ancestry before writing anything, and emits numbered `receipt-NNNN.md` comment files (into `--out <dir>` when given, else the current directory; emitted paths and printed commands are absolute so `--post` and pasted commands work from any directory, with the body-file argument shell-quoted when needed; a mid-sequence write failure removes the already-written files so no partial set is left) that pass the existing evidence grammar by construction; newly assembled receipts declare `Protocol: evidence/v2` and carry bounded excerpts beside each run output's byte count and SHA-256 in a single comment, while the decision-0008 continuation and chunk machinery remains only for retained earlier-protocol receipts (decision 0012). `--issue <N>` also prints the exact `gh issue comment` commands in posting order; `--queue <path>` emits files only. Posting is strictly opt-in: `--post` runs the printed commands itself in posting order, reporting each posted comment, and a failing gh invocation stops the chain naming the failing command, the posted parts, and the unposted files with no retry or reordering — the CLI's first gh write path. It never ticks checkboxes — the build skill's hand-assembly protocol remains valid until a later change mandates the CLI. Optional `--rebuild` and `--recovered-from` assemble rebuild routing identity and recovery provenance into the same canonical Receipt ID
- **`litespec issue check`** ticks exactly one unit checkbox in a GH issue body: it resolves the unit by exact heading and positive same-heading occurrence (digest identity semantics), flips exactly one `- [ ]` to `- [x]`, and writes back through `gh issue edit` only when the result differs from the fetched body by that single flip with the `Base:`/`Branch:` ownership lines byte-unchanged. Any other body delta, an ambiguous or unknown heading, an out-of-range occurrence, an already-checked target, or a gh failure is a visible refusal that issues no write. There is deliberately no offline queue lane and no broader body management — hand-editing issue bodies is retired
