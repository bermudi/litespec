...
- **Units** — one demo-able outcome per `##` with `Done means:` and `Verify:` that must fail without the outcome, plus optional `Read first:` (context, not scope — areas/rulings, not file lists) and `Constraints:` (boundaries: what must stay true or out of bounds — never what to edit; omit rather than placeholder) and `Depends:`. Checked units must carry a nonempty `Evidence:` block scoped to that unit (GH issue: body or comments; local queue: `Evidence:` block after `Verify:`, before the checkbox). Unchecked units are unaffected. Evidence is verbatim — exact `Verify:` command, `HEAD` sha, exit status, raw output fence, and `Evidence scope: this command exited <status> at <sha>; nothing else is inferred.` — never prose. Built one at a time, ticked via checkbox only after evidence is posted. No `tasks.md`
...
- GH issue is the queue: each unit is `## <outcome>` with `Done means:` and `Verify:` and status checkbox, plus optional `Read first:` / `Constraints:` (both unique/nonempty, omit rather than placeholder) and `Depends:` and required `Evidence:` when checked (body or comments for GH, `Evidence:` block for local)
