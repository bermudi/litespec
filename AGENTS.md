...
- `build` implements one unit per session: from a clean tree it runs the exact `Verify:` before implementation and requires a meaningful non-zero pre run. If the verifier does not exist, it may create at most one verifier-only commit. It selects unchecked units or checked GitHub units with unresolved rebuild requests, then creates one or more implementation/fix commits, never amends them, and records post at the final clean commit where `Verify:` passes. It posts both runs as a verbatim receipt; rebuild receipts carry exact heading + heading occurrence and leave the checked body unchanged. Local-queue bookkeeping is a separate metadata commit. Stops
