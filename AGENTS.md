...
- **Units** — one demo-able outcome per `##` with `Done means:` and `Verify:` that must fail without the outcome, plus optional `Read first:` (context, not scope — areas/rulings, not file lists) and `Constraints:` (boundaries: what must stay true or out of bounds — never what to edit; omit rather than placeholder) and `Depends:`. Checked units must carry a complete red-green evidence receipt scoped to that unit (GH issue: body or comments; local queue: `Evidence:` block after `Verify:`, before the checkbox) whose `unit digest:` (printed by `litespec digest`) binds the receipt to the unit's contract fields. One exact Verify is recorded at two immutable clean commits: a non-zero pre run because the outcome is absent, then a zero-status post run at the final clean commit where `Verify:` passes. Each run has a full SHA, integer status, fenced raw output, and matching conservative scope line. A checked GitHub unit with an unresolved structured rebuild-request comment remains selectable; a later complete receipt with the same exact heading and 1-based same-heading occurrence resolves all earlier requests. Only plan may change a unit contract — via an append-only `Amendment:` record whose New digest reopens the unit until a receipt carrying it resolves it, and observed digests must chain over amendment edges to the current contract or validation fails. GitHub review never edits the body for rebuild routing. Local review unchecks only affected units in a clean metadata commit. A nonempty `Evidence:` label is not enough. Built one at a time. No `tasks.md`
...
- `build` implements one unit per session: from a clean tree it runs the exact `Verify:` before implementation and requires a meaningful non-zero pre run. If the verifier does not exist, it may create at most one verifier-only commit. It selects unchecked units or checked units with unresolved rebuild requests or amendment records, then creates one or more implementation/fix commits, never amends them, and records post at the final clean commit where `Verify:` passes. It posts both runs as a verbatim receipt carrying the unit digest from `litespec digest`; rebuild receipts carry exact heading + heading occurrence and leave the checked body unchanged. Local-queue bookkeeping is a separate metadata commit. Stops
- `review` is adversarial — context-aware check of GH issue + spec vs implementation, cross-checks every checked unit's pre/post receipt and ancestry, and replays Verify using a detached temporary worktree at pre, another at post, and a detached temporary worktree at `HEAD`; all are removed even when Verify fails. It then triages findings into lanes (DISPUTED is terminal non-blocking). For rule-2 GitHub rebuilds it posts one structured append-only request per affected unit identity and never edits the issue body; local queues still uncheck affected units in a clean metadata commit. Red-green discrimination does not prove that Verify targets the right behavior; review still judges that. (See below.)
...
`needs decision` is reported before applying the matching route and does not alter blocking status. The issue closes only when every unit checkbox is checked, every rebuild request is resolved, and review returns `PASS`. Closure additionally requires every observed receipt `unit digest:` to chain to its unit's current contract through plan-authored amendments (decision 0006) — validate fails on unbridged digest transitions.
