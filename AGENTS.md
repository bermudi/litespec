...
- **Skills** are generated into `.agents/skills/` (canonical). Nearly all AI coding agents (opencode, Cursor, Windsurf, Amazon Q, Auggie, Roo, Kilo Code, Codex, etc.) now discover `.agents/skills/` natively. Claude Code is the sole exception: it reads from `.claude/skills/`, so `--tools claude` creates symlinks there. **Project-specific skills** (`skill-creator`, `the-drill`) are tracked directly in git as plain markdown — they are NOT generated by the binary. Only `litespec-*` skills are generated via `litespec update`. The `litespec-*` namespace is reserved — do not create custom skills with that prefix; `litespec update` will delete them.
...
- **review** is context-aware AI review that adapts to change lifecycle: the `reviewMode` field in `status --json` tells the agent which mode to use ("artifact", "implementation", or "pre-archive"). Artifact review evaluates planning artifacts, implementation review runs adversarial then compliance review, pre-archive review adds archive readiness and build verification. Adversarial review runs first to avoid anchoring bias — it enumerates failure scenarios from specs before reading code. No test/lint running (except build verification in pre-archive mode).
...
**Domain skills** (`litespec-think`, `litespec-plan`, `litespec-build`, `litespec-review`) are generated by the Go binary. The pipeline is:
...
3. Run `litespec update` — generates `.agents/skills/<name>/SKILL.md` from the `SkillInfo` metadata + registered template. Skills can also include resource files in `internal/skill/templates/references/<id>/`, which are generated into `.agents/skills/<name>/references/`.
