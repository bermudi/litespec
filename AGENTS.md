...
- `build` implements one unit per session: from a clean tree it runs the exact `Verify:` before implementation and requires a meaningful non-zero pre run. If the verifier does not exist, it may create at most one verifier-only commit. It selects unchecked units or checked units with unresolved rebuild requests or amendments, but refuses unresolved re-plan markers. It treats scenario/risk mappings as fixed contract fields, creates immutable implementation/fix commits, and records post at the final clean commit where `Verify:` passes. It posts both runs as a verbatim receipt carrying the unit digest from `litespec digest`; local-queue bookkeeping is a separate metadata commit. Stops
