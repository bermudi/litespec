...
- **Units** — one demo-able outcome per `##` with `Done means:` and `Verify:` that must fail without the outcome, plus optional `Read first:` (context, not scope — areas/rulings, not file lists) and `Constraints:` (boundaries: what must stay true or out of bounds — never what to edit; omit rather than placeholder) and `Depends:`. Checked units must carry a complete evidence receipt scoped to that unit (GH issue: body or comments; local queue: `Evidence:` block after `Verify:`, before the checkbox). Unchecked units are unaffected. A receipt is verbatim — exact `Verify:` command, labeled `sha:` (full 40- or 64-char hex), labeled `exit status: 0`, a fenced raw output block containing the command output or literal `<no output>`, and `Evidence scope: this command exited <status> at <sha>; nothing else is inferred.` — never prose. A nonempty `Evidence:` label is not enough. Built one at a time, ticked via checkbox only after the receipt is posted. No `tasks.md`
